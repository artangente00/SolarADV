...
@Service
public class ConductorTemperatureDerating {
	...
	public ConductorTemperatureDerating(CheckValueTypesService checkValue, TechnicalProblemMsg technicalProblem,
			CircuitEnvironment circuitEnvService, CircuitEnviromentMapping circuitEnvMapping, AmpacityCorrectionChart ampCorrection) {
		super();
	...
	public E002Model mapCondTempDerating(AcroFields form, int i, String acCircuit,
			Integer fourPerCentAverageHigh, List<String> acCircuitEnvironment, int sheetIndex, String state,
			E002Model params, PermitConduitConductorSectionEntity circuit, String necCode,
		...
		try {
			...
			form.setField(sheetIndex + "-" + "AC" + i + "-04-PERCENT-AVERAGE-HIGH-TEMPERATURE",String.valueOf(operatingTemperature));
			//A.B 04/11/2021 CR-3908
			form.setField(sheetIndex + "-" + "AC" + i
					+ "-HEIGHT-ABOVE-ROOF", circuitEnvService.getHighAboveRoof(params.getLastInvirement(), necCode, state));
			form.setField(sheetIndex + "-" + "AC" + i + "-TEMPERATURE-ADDER", circuitEnvService.getTempAdder(params.getLastInvirement(), necCode, state, userSetting, showConduitRoofAsHeight));
			form.setField(sheetIndex + "-" + "AC" + i + "-OPERATING-TEMPERATURE",operatingTemperature);
			form.setField(sheetIndex + "-" + "AC" + i + "-CONDUIT-TEMPERATURE-DERATING-AMPACITY-CORRECTION",acAmpacityCorrectionB2a);
			form.setField(sheetIndex + "-" + "AC" + i + "-CONDUIT-TEMPERATURE-DERATING-AMPACITY-CORRECTION1",acAmpacityCorrectionB2a);
