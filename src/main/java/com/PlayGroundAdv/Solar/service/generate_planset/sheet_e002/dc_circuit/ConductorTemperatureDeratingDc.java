...
@Service
public class ConductorTemperatureDeratingDc {

	final CheckValueTypesService checkValue;
	final TechnicalProblemMsg technicalProblem;
	final CircuitEnvironment circuitEnvService;

	public ConductorTemperatureDeratingDc(CheckValueTypesService checkValue, TechnicalProblemMsg technicalProblem,
	...
	public E002Model mapTemperatureDerating(int i, AcroFields form, String dcCircuit, Integer fourPerCentAverageHigh,
		...
		try {
			...
			if (checkValue.Equals(dcCircuit.split("-")[i - 1], "PV MODULE")) {
				form.setField(sheetIndex + "-" + "DC" + i + "-CIRCUIT-ENVIRONMENT", circuit.getCircuitEnvironment());
				params.setLastInvirement(circuit.getCircuitEnvironment());
			} else if (checkValue.Equals(dcCircuit.split("-")[i - 1], "DC/DC CONVERTER")
					|| checkValue.Equals(dcCircuit.split("-")[i - 1], "RAPID SHUTDOWN")) {
				form.setField(sheetIndex + "-" + "DC" + i + "-CIRCUIT-ENVIRONMENT", circuit.getCircuitEnvironmentTwo());
				params.setLastInvirement(circuit.getCircuitEnvironmentTwo());
			} else if (checkValue.Equals(dcCircuit.split("-")[i - 1], "JUNCTION BOX")) {
				form.setField(sheetIndex + "-" + "DC" + i + "-CIRCUIT-ENVIRONMENT",
						circuit.getCircuitEnvironmentThree());
				params.setLastInvirement(circuit.getCircuitEnvironmentThree());
			} else if (checkValue.Equals(dcCircuit.split("-")[i - 1], "DC COMBINER")) {
				form.setField(sheetIndex + "-" + "DC" + i + "-CIRCUIT-ENVIRONMENT",
						circuit.getCircuitEnvironmentFour());
				params.setLastInvirement(circuit.getCircuitEnvironmentFour());
			} else if (checkValue.Equals(dcCircuit.split("-")[i - 1], "DC DISCONNECT")) {
				form.setField(sheetIndex + "-" + "DC" + i + "-CIRCUIT-ENVIRONMENT",
						circuit.getCircuitEnvironmentFive());
				params.setLastInvirement(circuit.getCircuitEnvironmentFive());
			}
